...
> [!NOTE]
> **이 페이지에서 얻는 것**
> - 실제 보안팀 인력의 페르소나로 따라가는 4가지 심층 데모 시나리오
...
[Step 10 핸즈온 랩](./10-handson-lab.md)에서 기본기를 익혔다면, 이 페이지는 **가상의 보안팀 인력이 실제 업무를 처음부터 끝까지 수행**하는 과정을 따라갑니다. 등장인물과 상황은 학습용 가상이지만, **모든 기능·프롬프트·제약은 Microsoft 공식 문서에 근거**합니다. 각 시나리오 끝에는 남들이 잘 모르는 활용법을 정리한 **고급 활용 팁** 박스가 있습니다.

> [!IMPORTANT]
> 프롬프트의 인시던트 ID·사용자·시간 범위 등은 예시입니다. 실제로는 여러분 환경 값으로 바꾸세요. **프리뷰(Preview) 기능은 정식 출시 전이므로 결과를 반드시 검토**하고, 생성된 KQL·스크립트는 검증 없이 프로덕션에 적용하지 마세요. 각 기능의 GA/프리뷰 상태를 본문에 표기했습니다.

---

## 들어가기 전에: 좋은 프롬프트의 4요소

모든 시나리오의 프롬프트는 다음 네 가지를 담습니다. 데모를 따라가며 이 패턴을 눈여겨보세요.
...
## 시나리오 4 — 교차 운영: 위협 인텔 자동화 + IT 관리자 조사

> **👤 페르소나 — 위협 인텔 분석가 & IT 관리자**
>
> 위협 인텔 분석가는 매주 위협 브리핑을 손으로 만드느라 지쳐, **에이전트로 자동화**하려 합니다. 한편 IT 관리자는 특정 임원에 대한 이상 징후 제보를 받아, **4개 제품을 한 번에** 조사해야 합니다. 이 시나리오는 **자동화(에이전트)**와 **교차 제품 조사(프롬프트북)**라는, 실무에서 특히 임팩트가 큰 두 활용을 보여줍니다.

### 파트 A — 위협 인텔 브리핑 에이전트 자동화 (GA)

**Threat Intelligence Briefing Agent**를 하루가 아닌 **주기적 자동 실행**으로 설정합니다. Defender 포털 › 위협 인텔리전스 › **위협 분석(Threat analytics)** 상단 배너에서 시작합니다.

**설정 흐름**:
1. **Set up agent** → 신원 선택: **에이전트 ID 생성**(전용 Entra Agent ID, 권장) 또는 기존 사용자 계정 연결
2. 파라미터 구성 — Insights(조사할 취약점 수), Look back days(과거 조회 기간), Region(예: Europe), Industry(예: Financial Services), 예약(**기본 7일마다**), 브리핑 수신자(이메일/DL)
3. **Deploy agent** → **Run agent** → **View full brief**

**브리핑 내용**: 위협 요약 + 상세 기술 분석 + **활발히 악용되는 취약점**과 우리 조직 영향. 마크다운 다운로드/복사 가능하며, **활동 맵(activity map)**으로 에이전트가 밟은 단계를 투명하게 볼 수 있습니다.

> [!TIP]
> **에이전트 메모리**: 브리핑에 피드백(👍/👎)을 주면 **agent memory**에 저장되어 이후 출력에 반영됩니다. 에이전트 페이지 › (…) › **Manage memory**에서 저장된 피드백을 확인·삭제할 수 있습니다. 조직 취향을 "가르치는" 방법입니다. 리포트는 독립형 포털 **Activity**에 타임스탬프와 함께 보관됩니다.

### 파트 B — Microsoft User Analysis로 4개 제품 교차 조사 (GA)

임원 계정 하나에 대해 **Entra ID · Intune · Purview · Defender를 한 번에** 조사하는 프롬프트북입니다. IT 관리자에게 특히 유용합니다.

**흐름**: 프롬프트북 라이브러리 › **Microsoft User Analysis** › Start new session › **UPN + 시간 범위** 입력 › Submit. 프롬프트가 순차 실행되며 각 제품 데이터를 종합합니다.
- **Entra ID** — 로그인·인증 데이터
- **Intune** — 디바이스 정보
- **Purview** — 비정상 활동(내부자 위험)
- **Defender** — 주요 탐지 요약

**필요 최소 역할**: Entra·Intune·Defender는 Security Reader, Purview는 Insider Risk Management Investigator 또는 Analyst.

이어서 Intune Copilot로 디바이스 문제를 좁힙니다. **정상 기기와 문제 기기를 비교**하는 활용이 특히 강력합니다.

> 🇰🇷 "DeviceA와 DeviceB의 하드웨어 구성 차이가 뭐야?"
>
> 🇺🇸 *`What is the hardware configuration difference between the DeviceA and DeviceB devices?`*

> 🇰🇷 "왜 PolicyA가 DeviceB에 적용되고 있어?"
>
> 🇺🇸 *`Why is PolicyA applying on DeviceB?`*

> 🇰🇷 "이 오류 코드를 분석해 줘." (오류 코드 입력 → 설명 + 해결책)
>
> 🇺🇸 *`Analyze an error code`*

**결과**: 위협 인텔 분석가는 반복 업무를 에이전트로 자동화했고, IT 관리자는 4개 제품 조사와 디바이스 비교 진단을 몇 번의 클릭으로 끝냈습니다.

> [!TIP]
> **고급 활용 팁 — 잘 모르는 사용법**
> - **Intune 정책 구성 에이전트(GA)**: CIS 벤치마크 같은 **문서·기준선을 붙여넣으면** 그에 맞는 Intune 설정 카탈로그 값을 찾아 **정책 초안**으로 만들어 줍니다.
> - **프롬프트에 "Intune" 넣기**: "According to Intune, …", "이번 주에 며칠 동안 몇 대가 등록됐어?"처럼 제품명을 넣으면 정확도가 올라갑니다.
> - **에이전트 신원은 전용 Agent ID로**: 파트너 에이전트는 설정 전 전역 관리자 승인이 필요하며, Microsoft 제작 에이전트는 **전용 Entra Agent ID**로 접근 범위를 좁게 유지하는 것이 권장됩니다.

참고: [위협 인텔 브리핑 에이전트](https://learn.microsoft.com/defender-xdr/threat-intel-briefing-agent-defender) · [에이전트 개요](https://learn.microsoft.com/security-copilot/agents-overview) · [프롬프트북 사용](https://learn.microsoft.com/security-copilot/using-promptbooks) · [Intune에서 Copilot](https://learn.microsoft.com/intune/intune-service/copilot/copilot-intune-overview)
