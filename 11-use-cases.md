...
이제 이 인시던트 범위를 넘어, 같은 피싱에 당했거나 민감 데이터를 반출당한 또 다른 사용자가 있는지 찾고자 합니다. Advanced Hunting의 **Query Assistant(NL-to-KQL, GA)**로 개별 쿼리를 만들거나, **Threat Hunting Agent(Preview)**로 대화형 헌팅을 합니다.
