...
| 경로 | 방법 |
| --- | --- |
| **활성 세션 내에서** | ✨ **Prompts** 아이콘 클릭 → **Promptbooks**로 필터 → **Get started** |
| **홈 메뉴에서** | **Promptbooks** → 프롬프트북 라이브러리(Promptbook library) |

![프롬프트북 라이브러리](./images/04-promptbook-library.png)
*프롬프트북 라이브러리 — 기본 제공 및 커스텀 프롬프트북을 탐색하고 실행합니다.*

## 3. 기본 제공 프롬프트북 (GA)

기본 제공 프롬프트북은 모두 정식 출시(GA) 상태이며, 각각 **특정 플러그인 또는 입력**을 필요로 합니다.
...
자주 사용하는 프롬프트 흐름을 재사용 가능한 프롬프트북으로 만들 수 있습니다.
...
> [!TIP]
> `<ParameterName>` 매개변수는 각괄호로 감싸고 공백을 넣지 않습니다. 프롬프트북 실행 시 이 매개변수 값을 입력받아 여러 프롬프트에 걸쳐 재사용합니다.
